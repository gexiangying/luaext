...
local function add_nodes()
	for k, v in pairs(s2k.model.joints) do
		local nd = v.id
		local pt = {}
		pt.x = v.coord[1]*1000
		pt.y = v.coord[2]*1000
...
local function add_mems()
	for k, v in pairs(s2k.model.members) do
		local m = {}
		m.start_point = {}
		m.end_point = {}
		...
		-- local jpt = s2k.model.joints[jid].coord
...
local function add_pt(pts, cids)
	for k, v in pairs(cids) do
		local jpt = s2k.model.joints[v].coord
		local pt = {}
		pt.x = jpt[1]*1000
		pt.y = jpt[2]*1000
		pt.z = jpt[3]*1000
...
local function add_pls()
	for k, v in pairs(s2k.model.elements) do
		local p = {}
