...
--select_ary = {}
model = {	lk = {0.11707,-356.0,-68.0,-1.0},
	texs={
...
--------------------------------------------
local stlcmd = require("steel_cmd");							-- better
local smd = require("steel_model");								-- better
smd.set_model(model);											-- better
--smd.set_select_ary(select_ary);									-- better

function begin_select()											-- better
--	luaext.msg("begin_select", "is_ctr_down()="..tostring(is_ctr_down() or "nil"))
--	if is_ctr_down() == 0 then
--		smd.model_select_ary(select_ary, nil)
--		luaext.msg("begin_select", "model_select_ary(nil)=")
--		select_ary = {}
--	end
end


function select_main(index)										-- better
--	local title = "select_main"..(index or "nil")
--	local ctr = is_ctr_down();
--	local ary = select_ary[index]
--	local msg = "is_ctr_down() = "..(ctr or "nil").."\nselect_ary[index] = "..(ary or "nil")
--	trace_out("--------"..title..msg);
--	luaext.msg(title, msg)
	if (is_ctr_down() ~= 0) and smd.get_select_ary_index(index) then
--		luaext.msg("smd.model_select", "nil")
		smd.model_select(index, nil)
	else
--		luaext.msg("smd.model_select", "1")
		smd.model_select(index, 1)
	end
	--trace_select()
end
...
--	scene_select(scene,x,y,10,10,1)
	stlcmd.on_lbuttondown(scene,flags,x,y)								-- better
end

function on_lbuttonup(scene,flags,x,y)
	stlcmd.on_lbuttonup(scene,flags,x,y)								-- better
end
...
function on_command(cmd,child)
	...
	elseif(cmd == ID + 6) then
		...
		--test_iup2()
	elseif stlcmd.on_command(cmd) then							-- better
